...
/// A _thin_ wrapper around the Murmur3 hash function so it can support `Hasher` and `Hash` traits
///
/// For the `write` and `finish` implementations, whatever input is provided is immediately evaluated. There is no complex internal state for this implementation.
...
impl Murmur3Hasher {
    ...
    /// Wrapper method around the hash function
    pub fn hash(&self, bytes: &[u8]) -> u128 {
        murmur3_x86_128(bytes, self.seed)
    }
}
